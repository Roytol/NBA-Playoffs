...
export async function syncPlayoffSeries(season = CURRENT_SEASON) {
  ...
  console.log(
    `[nbaSync] Sync complete. Created: ${created}, Updated: ${updated}`,
  );
...
async function processMatchup(
  ...
  const team1Conf =
    seedMap.get(team1Name)?.conference || firstGame.home_team.conference || "";
  const team2Conf =
    seedMap.get(team2Name)?.conference ||
    firstGame.visitor_team.conference ||
    "";
  ...
  if (team1Seed === 0 && team2Seed === 0 && games.length > 0) {
    const team1HomeGames = games.filter(
      (g) => g.home_team.full_name === team1Name,
    ).length;
    const team2HomeGames = games.filter(
      (g) => g.home_team.full_name === team2Name,
    ).length;
...
function findCurrentGame(games) {
  ...
  const liveGame = games.find((g) => {
    const isISO =
      g.status && (g.status.includes("T") || g.status.includes("Z"));
    return (
      g.status &&
      g.status !== "Final" &&
      g.status !== "" &&
...
function formatGameData(game, allGames) {
  ...
  const isISO =
    game.status && (game.status.includes("T") || game.status.includes("Z"));
  const isLive =
    game.status &&
    game.status !== "Final" &&
    game.status !== "" &&
...
function detectRound(
  ...
  const seedPair = [
    Math.min(team1Seed, team2Seed),
    Math.max(team1Seed, team2Seed),
  ];
...
export async function updateLiveScores(liveGames) {
  ...
  for (const series of activeSeries) {
    ...
    if (matchingGame) {
      const isISO =
        matchingGame.status &&
        (matchingGame.status.includes("T") ||
          matchingGame.status.includes("Z"));
      const isLive =
        matchingGame.status &&
        matchingGame.status !== "Final" &&
        !isISO &&
        matchingGame.period > 0;
      ...
      if (
        JSON.stringify(series.current_game) !== JSON.stringify(currentGameData)
      ) {
        await supabase
          .from("Series")
