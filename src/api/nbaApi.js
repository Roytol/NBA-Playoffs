...
export async function getTeams() {
  const teams = await getFromCacheOrFetch(
    "teams",
    CACHE_TTL.TEAMS,
    async () => {
...
export async function getPlayoffGames(season = CURRENT_SEASON) {
  return getFromCacheOrFetch(
    `games_postseason_${season}`,
    CACHE_TTL.PLAYOFF_GAMES,
    async () => {
 ...
 */
export async function getHeadToHeadMatchups(
  team1Name,
  team2Name,
  ...
  const cacheKey = `h2h_${season}_${sortedNames[0]}_${sortedNames[1]}`.replace(
    /\s+/g,
    "",
  );
  ...
  return getFromCacheOrFetch(cacheKey, H2H_TTL, async () => {
    try {
      ...
      const t1 = teams.find(
        (t) => t.full_name === team1Name || t.name === team1Name,
      );
      const t2 = teams.find(
        (t) => t.full_name === team2Name || t.name === team2Name,
      );
      ...
      const games = h2hGames.map((game) => {
        ...
        const t1Score = isT1Home
          ? game.home_team_score
          : game.visitor_team_score;
        const t2Score = isT1Home
          ? game.visitor_team_score
          : game.home_team_score;
...
export async function getTeamNames() {
  try {
    const teams = await getTeams();
    return teams.map((t) => t.full_name).sort();
  } catch {
