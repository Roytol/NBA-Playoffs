...
export function bracketSortKey(series, roundKey) {
  ...
  if (roundKey === "first_round") {
    const idx = FIRST_ROUND_DISPLAY_PAIRS.findIndex(
      (p) => p[0] === lo && p[1] === hi,
    );
    if (idx !== -1) return idx;
  ...
  if (roundKey === "second_round") {
    const inUpper =
      s1 > 0 && s2 > 0 && UPPER_HALF_SEEDS.has(s1) && UPPER_HALF_SEEDS.has(s2);
    const inLower =
      s1 > 0 && s2 > 0 && LOWER_HALF_SEEDS.has(s1) && LOWER_HALF_SEEDS.has(s2);
...
export function sortSeriesForBracketDisplay(seriesList, roundKey) {
  return [...seriesList].sort((a, b) => {
    ...
    return (
      Math.max(a.team1_seed ?? 0, a.team2_seed ?? 0) -
      Math.max(b.team1_seed ?? 0, b.team2_seed ?? 0)
    );
  });
}
