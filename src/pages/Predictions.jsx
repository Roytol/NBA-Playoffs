...
export default function PredictionsPage() {
  ...
  const handleUpdateChampion = async () => {
    ...
    try {
      ...
      toast({
        title: "Champion pick saved! 🏆",
        description: championForm.champion,
      });
    } catch (err) {
      console.error("Error updating champion:", err);
      toast({
        title: "Failed to save champion pick",
            ...
            <DialogDescription>
              ...
              {championMVPDeadline && (
                <div className="mt-2 text-xs flex items-center gap-1">
                  <Clock className="w-3 h-3" />
                  Deadline:{" "}
                  {format(championMVPDeadline, "MMM d, yyyy 'at' h:mm a")}
                </div>
              )}
            </DialogDescription>
          </DialogHeader>
