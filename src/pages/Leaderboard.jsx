...
export default function LeaderboardPage() {
  ...
  const pastSeasonRequestRef = React.useRef(0);
  ...
  const loadSeasonMeta = async () => {
    try {
      ...
      const seasonSetting = settings.find(
  ...
  const loadPastSeasonLeaderboard = async (season) => {
    const requestId = pastSeasonRequestRef.current + 1;
    pastSeasonRequestRef.current = requestId;
    setPastLoading(true);
    try {
      ...
      for (const pred of data || []) {
        ...
        userTotals[pred.user_email] =
          (userTotals[pred.user_email] || 0) + (pred.points_earned || 0);
      }

      // Fetch user names
      const users = await listUsers();
      const userMap = Object.fromEntries(
        users.map((u) => [u.email, u.full_name]),
      );
      ...
      if (pastSeasonRequestRef.current !== requestId) return;
      ...
      if (pastSeasonRequestRef.current === requestId) {
  ...
  const loadLeaderboard = async () => {
    ...
    try {
      ...
      const latest = sorted.reduce((latest, e) => {
        if (
          e.last_updated &&
          (!latest || new Date(e.last_updated) > new Date(latest))
        )
        ...
        {allTabs.length > 1 && (
          <TabsList className="mb-4">
            <TabsTrigger value="current">
              {activeSeason
                ? `${formatSeasonLabel(activeSeason)} Season`
                : "Current Season"}
            </TabsTrigger>
                ...
                <Table>
                  <TableHeader className="bg-gray-50">
                    <TableRow>
                      ...
                      <TableHead className="py-2 px-3 sm:px-4 text-xs sm:text-sm">
                        Player
                      </TableHead>
                      <TableHead className="text-center py-2 px-3 sm:px-4 text-xs sm:text-sm">
                        Points
                      </TableHead>
                    </TableRow>
                  </TableHeader>
                        ...
                        >
                          ...
                          <TableCell className="py-2 px-3 sm:px-4 text-xs sm:text-sm">
                            <div className="font-medium truncate max-w-[120px] sm:max-w-none flex items-center gap-1.5">
                              ...
                              {entry.hotStreak >= 2 && (
                                <span
                                  title={`${entry.hotStreak} exact series picks in a row!`}
                                  className="cursor-help animate-bounce drop-shadow-md pb-1 text-sm"
                                >
                                  🔥
                                </span>
                              )}
                              {entry.coldStreak >= 2 && (
                                <span
                                  title={`${entry.coldStreak} completed-series picks in a row with 0 points`}
                                  className="cursor-help drop-shadow-md pb-1 text-sm"
                                >
                                  🧊
                                </span>
                              )}
                    ...
                    <Table>
                      <TableHeader className="bg-gray-50">
                        <TableRow>
                          ...
                          <TableHead className="text-center py-2 px-3 text-xs">
                            Points
                          </TableHead>
                        </TableRow>
                      </TableHeader>
