...
export default function UserPredictionsPage() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const [predictionsData, seriesData, settings, leaderboardData] =
        await Promise.all([
      ...
      const validPredictions = predictionsData.filter((p) => {
        if (p.series_id) {
          const relatedSeries = seriesData.find(
            (s) => s.series_id === p.series_id,
          );
          return (
            relatedSeries && new Date(relatedSeries.prediction_deadline) < now
          );
        }
      ...
      if (leaderboardData.length > 0) {
        setUserData({
          ...
          points: validPredictions.reduce(
          ...
          <Card>
            <CardHeader className="py-2 px-3 sm:py-4 sm:px-6">
              <CardTitle className="flex items-center gap-2 text-sm sm:text-base">
                ...
                {activeTab === "all"
                  ? "All Predictions"
                  : `${getRoundDisplayLabel(activeTab)} Predictions`}
              </CardTitle>
            </CardHeader>
            <CardContent className="p-0">
              <Table>
                <TableHeader>
                  <TableRow>
                    <TableHead className="py-2 px-2 sm:px-4 text-xs">
                      Series
                    </TableHead>
                    <TableHead className="py-2 px-2 sm:px-4 text-xs">
                      Pick
                    </TableHead>
                    <TableHead className="py-2 px-2 sm:px-4 text-xs text-center">
                      Pts
                    </TableHead>
                ...
                <TableBody>
                  {filteredPredictions.length === 0 ? (
                    ...
                    filteredPredictions.map((prediction) => {
                      const seriesInfo = prediction.series_id
