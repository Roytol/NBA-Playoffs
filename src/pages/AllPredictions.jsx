...
export default function AllPredictionsPage() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const [seriesData, predictionsData, leaderboard, settings] =
        await Promise.all([
          listSeries(),
          listPredictions(),
          listLeaderboardEntries(),
      ...
      const championDeadline = settings.find(
        (s) => s.setting_name === SETTINGS_KEYS.CHAMPION_PREDICTION_DEADLINE,
      )?.setting_value;
      ...
      const closedSeries = seriesData.filter(
        (s) => new Date(s.prediction_deadline) < now,
      );
      ...
      const enhancedPredictions = validPredictions.map((prediction) => {
        const leaderboardEntry = leaderboard.find(
          (l) => l.player_id === prediction.user_email,
        );
        return {
          ...prediction,
  ...
  return (
    ...
    >
      ...
      >
        ...
        <p className="text-gray-500 mb-4 sm:mb-6 text-sm">
          View all user predictions for closed series
        </p>
      </motion.div>
            ...
            <TabsContent value={activeRound}>
              <div className="space-y-3">
                <AnimatePresence mode="popLayout">
                        ...
                        >
                              ...
                              {group.seriesInfo ? (
                                <div className="flex items-center gap-1 sm:gap-2 flex-wrap text-sm sm:text-base">
                                  <span className="hidden sm:inline">
                                    {group.seriesInfo.conference}
                                  </span>
                                  <span className="sm:hidden">
                                    {group.seriesInfo.conference === "East"
                                      ? "E"
                                      : "W"}
                                  </span>{" "}
                          ...
                          {group.seriesInfo && (
                            <div className="mt-1 sm:mt-2 space-y-1 text-xs sm:text-sm">
                              <div className="flex items-center gap-1 sm:gap-2">
                                <TeamLogo
                                  team={group.seriesInfo.team1}
                                  className="w-5 h-5 sm:w-6 sm:h-6"
                                />
                                <span>
                                  {group.seriesInfo.team1} (
                                  {group.seriesInfo.team1_seed})
                                </span>
                              </div>
                              <div className="flex items-center gap-1 sm:gap-2">
                                <TeamLogo
                                  team={group.seriesInfo.team2}
                                  className="w-5 h-5 sm:w-6 sm:h-6"
                                />
                                <span>
                                  {group.seriesInfo.team2} (
                                  {group.seriesInfo.team2_seed})
                                </span>
                              </div>
                            </div>
                          )}
                              ...
                              <Table>
                                <TableHeader>
                                  <TableRow>
                                    ...
                                    <TableHead className="py-2 px-3 text-xs text-center">
                                      Pts
                                    </TableHead>
                  ...
                  {Object.entries(groupPredictionsByType(filteredPredictions))
                    .length === 0 && (
                    <motion.div
                      initial={{ opacity: 0 }}
                      animate={{ opacity: 1 }}
